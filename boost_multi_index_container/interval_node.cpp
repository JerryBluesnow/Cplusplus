...
IntervalTNode *
IntervalT_Search(IntervalTree *T, interval i)
{
...
void
IntervalT_InorderWalk(IntervalTNode *x)
{
    if (x != NIL)
    {
...
IntervalTNode *
IntervalT_Minimum(IntervalTNode *x)
{
    while (x->left != NIL)
        x = x->left;
    return x;
...
IntervalTNode *
IntervalT_Successor(IntervalTNode *x)
{
    if (x->right != NIL)
...
void
Left_Rotate(IntervalTree *T, IntervalTNode *x)
{
...
void
Right_Rotate(IntervalTree *T, IntervalTNode *x)
{
...
void
IntervalT_InsertFixup(IntervalTree *T, IntervalTNode *z)
{
...
void
IntervalT_Insert(IntervalTree *T, interval inte)
{
...
void
IntervalT_DeleteFixup(IntervalTree *T, IntervalTNode *x)
{
    IntervalTNode *w;
    while (x != T->root && x->color == BLACK)
    {
        if (x == x->parent->left)
        {
            ...
            else
            {
                ...
                w->color =
                    x->parent->color;  // case 4: x's sibling w is black,and w's right child is red
...
void
IntervalT_Delete(IntervalTree *T, IntervalTNode *z)
{
...
main()
{
    interval A[] = {{16, 21}, {8, 9},   {25, 30}, {5, 8},  {15, 23},
    ...
    else
    {
        ...
        IntervalT_InorderWalk(T->root);
        cout << "The root of the tree is:" << T->root->inte.low << "   " << T->root->inte.high
             << endl;
    }
    cout << "/*------------------------------------------------------------*/" << endl;
