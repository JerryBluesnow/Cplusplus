...
class acl_list_address_index
{
   public:
    ...
    acl_list_address_index(unsigned int tbl_id, range_index<unsigned int> ip_rang, range_index<unsigned int> port_rang, unsigned vl_id)
...
{
    cout << "LEFT data to be compared"
         << ":" << dec << lhs.acl_tabl_id 
         << "," << hex << lhs.ip_suffix_range.index_start
         << "," << hex << lhs.ip_suffix_range.index_end 
         << "," << dec << lhs.port_range.index_start 
         << "," << dec << lhs.port_range.index_end << endl;

    cout << "RIGHT data to be compared"
         << ":" << dec << rhs.acl_tabl_id 
         << "," << hex << rhs.ip_suffix_range.index_start
         << "," << hex << rhs.ip_suffix_range.index_end 
         << "," << dec << rhs.port_range.index_start 
         << "," << dec << rhs.port_range.index_end << endl;
...
std::ostream& operator<<(std::ostream& os, const acl_list_db* acl_db_ptr)  
{
    ...
    os << dec <<acl_db_ptr->id << ", " 
       << dec << acl_db_ptr->myIndex.acl_tabl_id << ", ("
       << hex << acl_db_ptr->myIndex.ip_suffix_range.index_start << ", "
       << hex << acl_db_ptr->myIndex.ip_suffix_range.index_end << "), ("
       << dec << acl_db_ptr->myIndex.port_range.index_start << ", "
       << dec << acl_db_ptr->myIndex.port_range.index_end << ") "
       << "- " << endl;
    return os;  
}  
...
void test_insert()  
{
    ...
    acl->insert(new acl_list_db(14, range_index<unsigned int>(0x2001, 0x3fff), range_index<unsigned int>(1230, 1240), 0, acl_list_attributes(), 1));
    acl->insert(new acl_list_db(14, range_index<unsigned int>(0x3001, 0x3fff), range_index<unsigned int>(1000, 1200), 0, acl_list_attributes(), 2));
    acl->print(); 
}
