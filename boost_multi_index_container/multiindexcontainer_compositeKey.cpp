...
struct x_key : composite_key<MyTest, BOOST_MULTI_INDEX_MEMBER(MyTest, int, x)>
{
};
struct xy_key : composite_key<MyTest,
                              BOOST_MULTI_INDEX_MEMBER(MyTest, int, x),
                              BOOST_MULTI_INDEX_MEMBER(MyTest, int, y)>
{
};
struct xyz_key : composite_key<MyTest,
                               BOOST_MULTI_INDEX_MEMBER(MyTest, int, x),
                               BOOST_MULTI_INDEX_MEMBER(MyTest, int, y),
...
typedef multi_index_container<
    MyTest *,
    indexed_by<ordered_non_unique<tag<MyIndex_X_Tag>,
                                  x_key,
                                  composite_key_result_less<x_key::result_type> >,
               ordered_non_unique<tag<MyIndex_XY_Tag>,
                                  xy_key,
                                  composite_key_result_less<xy_key::result_type> >,
               ordered_unique<tag<MyIndex_XYZ_Tag>,
                              xyz_key,
                              composite_key_result_less<xyz_key::result_type> > > >
    MyContainer_T;
...
void
query_container(int x, int y, int z)
{
...
void
print_container(const MultiIndexContainer &container)
{
    // obtain a reference to the index tagged by Tag
    const typename boost::multi_index::index<MultiIndexContainer, Tag>::type &indexSet =
        get<Tag>(container);
...
void
free_container(MultiIndexContainer &container)
{
    typedef typename MultiIndexContainer::value_type value_type;
