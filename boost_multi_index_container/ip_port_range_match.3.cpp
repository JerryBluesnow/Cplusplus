...
operator==(const acl_list_address_index &lhs, const acl_list_address_index &rhs)
{
    ...
    if (lhs.ip_prefix > rhs.ip_prefix)
        return false;
    if (lhs.ip_suffix_range.index_start < rhs.ip_suffix_range.index_start)
        return true;
    if (lhs.ip_suffix_range.index_start > rhs.ip_suffix_range.index_start)
        return false;
    if (lhs.ip_suffix_range.index_end < rhs.ip_suffix_range.index_end)
        return true;
    if (lhs.ip_suffix_range.index_end > rhs.ip_suffix_range.index_end)
        return false;
    if (lhs.port_range.index_start < rhs.port_range.index_start)
        return true;
    if (lhs.port_range.index_start > rhs.port_range.index_start)
        return false;
    if (lhs.port_range.index_end < rhs.port_range.index_end)
        return true;
    if (lhs.port_range.index_end > rhs.port_range.index_end)
        return false;
...
main()
{
    ...
    mycontainer.find(acl_list_address_index(172018000,  range_index(1, 1), range_index(1230, 1230)));
